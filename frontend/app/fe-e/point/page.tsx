...
export default function PointRewardPage() {
  ...
  const streak = {
    ...
    progressPercent: Math.min((streakData.currentStreakDays / 1000000) * 100, 100)
  };
      ...
      <div className="mx-auto flex w-full min-h-[100dvh] sm:min-h-0 sm:h-[740px] max-w-none sm:max-w-sm flex-col sm:rounded-3xl bg-white px-5 py-6 sm:shadow-[0_8px_30px_rgba(31,42,37,0.06)] overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] pt-[12px] pb-[100px] flex flex-col gap-4">
          ...
          <div className="mt-[4px] flex flex-col shrink-0 bg-white rounded-[24px] p-[16px] shadow-[0_2px_16px_rgba(0,0,0,0.04)] border border-gray-50">
            {/* 상단 타이틀과 응모권 모으기 버튼 배치 */}
            <div className="flex items-center mb-[12px]">
              <h2 className="text-[16px] font-extrabold text-[#000000]">최근 적립</h2>
              
              <button 
                onClick={() => router.push('/fe-e/ticket')} 
                className="mr-auto text-[12px] font-bold text-[#41C0A1] bg-[#EAF9F4] px-3 py-1.5 rounded-full hover:bg-[#d4f2e9] transition-colors"
              >
                응모권 모으기
              </button>
            </div>
            ...
            <div className="flex flex-col gap-[14px]">
              {point.transactions.length > 0 ? (
                point.transactions.map((tx) => (
                  <div key={tx.transactionId} className="flex items-start justify-between">
                    
                    {/* 좌측 영역 (아이콘 + 텍스트 그룹) */}
                    <div className="flex items-start gap-[12px]">
                      <div className={`w-[36px] h-[36px] rounded-full flex items-center justify-center shrink-0 border ${
                      ...
                      <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                          <span className={`text-[15px] font-extrabold whitespace-nowrap ${tx.isEarn ? 'text-[#41C0A1]' : 'text-[#FF5C5C]'}`}>
                            {tx.formattedAmount} P
                          </span>
                          <span className="text-[14px] text-[#555555] font-medium">{tx.reason}</span>
                        </div>
                        <span className="text-[12px] text-[#999999] font-medium mt-[1px]">{tx.createdAt}</span>
                      </div>
                    </div>
