...
1. **클립을 찾을 수 없을 때의 오류 코드**: 명세서 6장 공통 오류 코드에는 "클립 없음"에 대응하는
   코드가 없습니다. `mission_clips`가 미션 하위 리소스인 점을 고려해 `CLIP-001`(404)을 자체 추가했습니다.
   `MISSION-001`을 재사용할지, 정식으로 스펙에 추가할지 협의가 필요합니다.
2. **미션 유효 시간대 검증(MISSION-004, 18.3)**: 업로드 요청(`missionId`, `clip`, `shared`)에 시간대
   정보가 없고, BE B는 `missions` 테이블에 접근하지 않습니다. 현재는 이 검증을 구현하지 않았습니다
   (모든 시간대를 통과시킴). BE A/BE C와 미션 유효 시간대를 어떻게 BE B에 전달할지(요청 파라미터 추가,
   내부 조회 API 등) 협의가 필요합니다.
3. **`mission_id`의 의미 범위**: 재촬영 한도(`retryCount` 등)를 "미션 인스턴스(사용자×날짜) 단위"로
   가정하고 구현했습니다 (명세서 12.4 "같은 미션 인스턴스" 문구 근거). 만약 `mission_id`가 미션
   "정의" ID라면 한 사용자의 실패가 다른 사용자의 재촬영 횟수까지 막게 되므로 반드시 확인이 필요합니다.
...
6. **판정 컨텍스트(mission_title, criteria)**: 업로드 요청에 미션 제목/판정 기준이 없어 BE B는
   미션 제목 없이 판정하고, AI가 매 요청마다 스스로 기준 id를 만듭니다. `ai-server`처럼 미션 정의에
   고정 criteria를 넣어 업로드 시 함께 내려주면 판정 정확도와 집계 품질이 좋아집니다.
